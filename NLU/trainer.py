...
class Trainer(object):
    ...
    def train(self):
        if os.path.exists(self.args.model_dir + "/training curves.csv"):
            os.remove(self.args.model_dir + "/training curves.csv")
        ...
        # save specs
        with open(self.args.model_dir + "/training specs.txt", 'w') as f:
            f.write("Num examples = %d\n" % len(self.train_dataset))
            f.write("Num Epochs = %d\n" % self.args.num_train_epochs)
            f.write("Total train batch size = %d\n" % self.args.train_batch_size)
            f.write("Gradient Accumulation steps = %d\n" % self.args.gradient_accumulation_steps)
            f.write("Total optimization steps = %d\n" % t_total)
            f.write("Logging steps = %d\n" % self.args.logging_steps)
            f.write("Save steps = %d\n" % self.args.save_steps)
    ...
    def evaluate(self, mode):
        ...
        intent_preds_list = [self.intent_label_lst[intent_id] for intent_id in intent_preds]
        ...
        with open(self.args.model_dir+'/output results.txt', 'w') as f:
            f.write('Intent predictions: \n')
            f.write(', '.join(intent_preds_list)+'\n')
        with open(self.args.model_dir+'/output results.txt', 'a') as f:
            f.write('\nSlot predictions: \n')
        ...
        if mode == 'dev':
            ...
            write_keys = False
            if not (os.path.exists(self.args.model_dir + "/training curves.csv")):
                write_keys = True

            with open(self.args.model_dir + "/training curves.csv", "a") as f:
                csv_writer = csv.writer(f)
                if write_keys:
                    csv_writer.writerow(list(sorted(results.keys())))
                csv_writer.writerow(res)

        # save eval result
        elif mode == 'test':
            with open(self.args.model_dir + "/eval.txt", 'w') as f:
                for key in sorted(results.keys()):
                    f.write( "%s = %s \n"%(key, str(results[key])) )
    ...
    def generate_results(self):
        # test_dataset serves as the input set
        dataset = self.test_dataset
